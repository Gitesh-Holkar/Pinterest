...
export const PinProvider = ({ children }) => {
  ...
  async function addComment(id, comment, setComment) {
    try {
      ...
      toast.success(data.message);
      fetchPin(id);
      setComment(""); // empty again the comment box
    } catch (error) {
      toast.error(error.response.data.message);
    }
  }
  ...
  return <PinContext.Provider value={{ pins, loading, fetchPin,fetchPins, pin, updatePin, addComment, deleteComment, deletePin, addPin }}>{children}</PinContext.Provider>
}
