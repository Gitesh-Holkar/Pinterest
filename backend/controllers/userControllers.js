...
export const registerUser = TryCatch(async (req, res) => {
  ...
  const hashPassword = await bcrypt.hash(password, 10); //it reutrn promises
...
export const loginUser = TryCatch(async (req, res) => {
  ...
  const comparePassword = await bcrypt.compare(password, user.password); //it reutrn promises
  if (!comparePassword)
    return res.status(400).json({
...
export const userProfile = TryCatch(async (req, res) => {
  const user = await User.findById(req.params.id).select("-password"); // :id from userRoutes is params
  res.json(user);
});
